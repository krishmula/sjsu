...
typedef struct {
  cond_type type;
  char col_name[MAX_IDENT_LEN + 1]; /* initially, before resolving */
  int col_index;                    /* index in combined column list */
  int relop;                        /* S_EQUAL, S_GREATER, S_LESS */
  bool is_int;
  bool is_string;
...
typedef struct {
  char name[MAX_IDENT_LEN + 1];
  int col_type; /* T_INT or T_CHAR */
  int col_len;
  int source;  /* 0 = left table, 1 = right table */
  int l_index; /* column index in left table, or -1 */
  int r_index; /* column index in right table, or -1 */
} combined_col_meta;
