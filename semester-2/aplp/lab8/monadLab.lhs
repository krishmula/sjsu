...
> applyOp Divide mi mj = mi >>= (\i -> mj >>= (\j -> if j == 0 then Nothing else Just $ i `div` j))
...
yes, using bind is better here. If we'd written this in do, it would've been much more verbose. And, using do, we also have to keep track of intermediate variables, which can be a hassle. So, using bind is more concise.
