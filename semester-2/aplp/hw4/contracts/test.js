...
describe('contract()', function() {
  it('should run the wrapped function when pre and post pass', function() {
    var f = contract([isNumber, isNumber], isNumber, function add(x, y) { return x + y; });
    assert.equal(f(2, 3), 5);
  });
  it('should throw blaming the caller when a pre-condition fails', function() {
    var f = contract([isNumber], isNumber, function id(x) { return x; });
    try {
      f('x');
      assert.fail('expected throw');
    } catch (e) {
      assert.ok(e.message.indexOf('Contract violation in position 0') === 0);
      assert.ok(e.message.indexOf('Top-level code') >= 0);
    }
  });
  it('should throw blaming the wrapped function when the post-condition fails', function() {
    function bad() { return 'nope'; }
    var f = contract([isNumber], isNumber, bad);
    try {
      f(1);
      assert.fail('expected throw');
    } catch (e) {
      assert.ok(e.message.indexOf('Blame -> bad') >= 0);
    }
  });
  it('should call pre and post with the same this as the wrapped call', function() {
    var pre = function(v) { return this && this.x === 1; };
    var post = function() { return true; };
    var o = { x: 1 };
    o.m = contract([pre], post, function() { return 0; });
    assert.equal(o.m(0), 0);
  });
});
