...
The \rel{B-Succ} and \rel{B-Pred} rules are for handling the successor and predecessor operations.
The premise for \rel{B-Succ} states that $e$ evaluates to some integer $i$.
If the premise holds, then the result of evaluating $\suc{e}$ is $i+1$.
The structure of \rel{B-Pred} is similar. If the premise holds, then the result of evaluating $\prd{e}$ is $i-1$.
